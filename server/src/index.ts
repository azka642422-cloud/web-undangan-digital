...
import { createOrder, createOrderSchema } from './orders.js';
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    const unavailable = code === 'DATABASE_NOT_CONFIGURED' || code === 'MIDTRANS_NOT_CONFIGURED';
    return res.status(unavailable ? 503 : 502).json({ ok: false, code: unavailable ? code : 'PAYMENT_PROVIDER_ERROR' });
  }
});
